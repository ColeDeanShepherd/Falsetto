    ...
    return (
      <Card>
        ...
        <div>
          {scales.map(scale => (
            <div>
              <NavLinkView to={`/scale/${getUriComponent(scale)}/lesson/introduction`}>{scale.rootPitchClass.toString(/*includeOctaveNumber*/ false)} {scale.type.name} Lesson</NavLinkView>
            </div>
          ))}
        </div>
        <br />
